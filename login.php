        ...
        if(!empty($login_email) && !empty($login_pw)) {
            ...
            if ($res) {
                ...
                if ($res && mysqli_num_rows($res) > 0) {
                    ...
                    if ($user_data['user_email'] == $login_email && password_verify($login_pw, $user_data['password'])) {
...
<div>
    ...
    <form method="post" id="login">
        <label>Email:</label>
        <input type="email" name="user_email" id="user_email" value="<?php echo htmlspecialchars($_POST['user_email'] ?? "") ?>" autofocus>
        <br>
        <br>
